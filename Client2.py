...
SAMPLE_PATH = "videos/samples/106/"
LEARNING_IMAGE_PATH = SAMPLE_PATH + "Ref.png"
...
class Client:
    ...
    def run(self):
        ...
        init_mte = False
        i = 0
        images_path = glob.glob(SAMPLE_PATH + "*.png")
        while i < len(images_path):
        # for i, image_path in enumerate(glob.glob(SAMPLE_PATH + "*.png")):
            # Sending
            ...
            if not sent_ref:
                print("Sending reference")
                self.mode = MTEMode.VALIDATION_REFERENCE
                sent_ref = True
            elif not init_mte:
                print("Initialize MTE")

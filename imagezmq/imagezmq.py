...
class ImageSender():
    ...
    def send_image_reqrep_image(self, msg, image):
        """Sends OpenCV image and msg to hub computer in REQ/REP mode
        ...
        Returns:
          A text and image reply from hub.
        """
        ...
        msg, jpg_buffer = self.zmq_socket.recv_array(copy=False)
...
class ImageHub():
    ...
    def send_reply_image(self, image, reply_message=b'OK'):
        """Sends the zmq REP reply message with an image.
        ...
        if image.flags['C_CONTIGUOUS']:
            # if image is already contiguous in memory just send it
            self.zmq_socket.send_array(image, reply_message, copy=False)
        else:
            # else make it contiguous before sending
            image = np.ascontiguousarray(image)
            self.zmq_socket.send_array(image, reply_message, copy=False)
